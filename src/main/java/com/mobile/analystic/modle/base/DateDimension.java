package com.mobile.analystic.modle.base;
...
public class DateDimension extends BaseDimension{
    ...
    private int week;
    private int day;
    private String type; //类型 年、月、天、周 指标类型
    private Date calendar = new Date(); //时间戳
    ...
    public static DateDimension buildDate(long time, DateEnum type){
        ...
        if(type.equals(DateEnum.SEASON)){
            ...
            calendar.set(year,month-1,1); //该季度的第一个月的一号
        ...
        if(type.equals(DateEnum.MONTH)){
            calendar.set(year,month-1,1); //该月的一号
            return new DateDimension(year,season,month,0,1,type.dateType,calendar.getTime());
        }
        //周指标
